...
Of the `r nrow(ip)` plant varieties that have been protected by PVPs or patents, we qualitatively coded a random subset of the plant descriptions (10%, `r nrow(ip)*.1`). Coding resulted in the identification of **X** themes: **[TO FILL IN]**. We used these `r nrow(ip)*.1` manual classification to train a neural network to classify plant description text into these categories, which is **X% effective**. The following categories are most prominent:
...
* `r length(unique(df$variety_name))` unique plant varieties and/or breeding lines are licensed, but only **[X, X%]** are protected by PVPs and patents. In many other cases the licensed materials are breeding lines/unfinished varieties (X%), or ?  
