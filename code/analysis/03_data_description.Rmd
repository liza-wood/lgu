...
## Whos is the main beneficiary of LGU licensing?

```{r, results = F, echo = F}
edges <- df %>% select(uni_state, licensee) %>% 
  filter(!is.na(licensee)) %>% 
  group_by(uni_state, licensee)# %>% 
  #count() %>% 
  #rename(weight = n)
nodes_uni <- df %>% select(uni_state, uni_region) %>% unique() %>% 
...
V(net)$deg <- log(igraph::degree(net, mode = "in"))+.5
table(V(net)$deg)
V(net)$label <- ifelse(V(net)$deg > 5, V(net)$name, "")
V(net)$label.size <- 10
...
```{r, out.width = "80%", fig.align= "center", eval = F}
df %>% 
