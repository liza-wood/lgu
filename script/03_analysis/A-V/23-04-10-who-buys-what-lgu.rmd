...
The crop licensed the most of all FAO classes is cereals, making up 3232 of the 7028  licenses. Washington dominates the cereal market at 2122 licenses, licensing 65.7% (2122/3232) of cereals. With this code, the percentage of any crop in any state out of all available license data can be calculated.
...
## Column data frame column names and descriptions:
  ...
  9) assets: wealth
  10) duns: ?
  11) address: business address
  12) naics2017: North American Industry Classification System
  13) yr_founded: year founded
  14) incorp_location: Additional location
  15) local_employee_number: number of employees at business location on license
  16) total_employee_number: vs global?
  17) global_employee_number: vs total?
  18) local_sales_number: gross income of business at location on license
  19) global_sales_number: total gross income of company
  20) db_type: ?
  21) licensee: official name from master company list on licenses spreadsheet (used for joining)
  22) zipcode: zip code
  23) lat: latitude coordinate of company location
  24) long: longitude coordinate of company location
  25) country: country of company HQ location
  26) state: US state of HQ
  27) domestic: logical to determine if international or not; TRUE means US based
  28) licensee_type: company or other
  29) intermediate_employee_number: irrelevant column for calculations only
  30) highest_employee_number: largest available data points for company employee number
  31) highest_sales_number: largest available data points for company sales money
  32) size_range_sales: determination of company size based on sales number
  33) size_range_employee: determination of company size based on employee number
...
licenses_joined_NA_sales %>% 
  group_by(top_uni, size_range_sales) %>% 
  count() %>% 
  ungroup() %>% 
  group_by(top_uni) %>% 
  mutate(percent = n/sum(n)) %>% 
  ggplot(aes(x = top_uni, y = n,  fill = size_range_sales)) + 
  geom_col() +
...
In conclusion, this analysis report allows for specific university action to be investigated further. Large companies play a role in the movement of grant money at land grant universities.
...
```{r}
write.csv(licenses_joined, "data_clean/licenses_joined.csv", row.names = F)
```
